...
async function main() {
	...
	if (process.env['BUILT_CLIENT']) {
		...
		const clientArchivePath = `.build/win32-${arch}/LoCoPilot-win32-${arch}.zip`;
		await $`7z.exe a -tzip ${clientArchivePath} ../LoCoPilot-win32-${arch}/* "-xr!CodeSignSummary*.md"`.pipe(process.stdout);
