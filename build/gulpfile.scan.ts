...
BUILD_TARGETS.forEach(buildTarget => {
	...
	const destinationExe = path.join(path.dirname(root), 'scanbin', `LoCoPilot${dashed(platform)}${dashed(arch)}`, 'bin');
	const destinationPdb = path.join(path.dirname(root), 'scanbin', `LoCoPilot${dashed(platform)}${dashed(arch)}`, 'pdb');
