...
async function main(buildDir?: string) {
	...
	const x64AppPath = path.join(buildDir, 'LoCoPilot-darwin-x64', appName);
	const arm64AppPath = path.join(buildDir, 'LoCoPilot-darwin-arm64', appName);
	const asarRelativePath = path.join('Contents', 'Resources', 'app', 'node_modules.asar');
	const outAppPath = path.join(buildDir, `LoCoPilot-darwin-${arch}`, appName);
