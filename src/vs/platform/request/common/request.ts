...
export interface IRequestToFileResult {
...
export interface IRequestService {
	 ...
	 * Use this for large downloads (e.g. language models) to avoid OOM.
	 * @param progressRequestIdOrOnProgress Renderer: pass requestId (subscribe to getRequestToFileProgressEvent first). Main process: receives callback from IPC.
	 */
	requestToFile?(options: IRequestOptions, destinationFilePath: string, token: CancellationToken, progressRequestIdOrOnProgress?: string | ((bytesReceived: number, contentLength: number | undefined) => void)): Promise<IRequestToFileResult>;

	/**
	 * Event for requestToFile progress. Subscribe with the same requestId before calling requestToFile.
	 */
	getRequestToFileProgressEvent?(requestId: string): Event<{ bytesReceived: number; contentLength: number | undefined }>;
