...
export class ChatService extends Disposable implements IChatService {
	...
	appendProgress(request: IChatRequestModel, progress: IChatProgress): void {
		const sessionResource = request.session.sessionResource;
		const model = this._sessionModels.get(sessionResource);
		this.logService.info(`[LoCoPilot Tool UI] ChatService.appendProgress: requestId=${request.id}, progress.kind=${progress.kind}, sessionResource=${sessionResource.toString()}, model=${model ? 'found' : 'null'}`);
		if (!(request instanceof ChatRequestModel)) {
