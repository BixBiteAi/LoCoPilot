...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	private getWelcomeViewContent(additionalMessage: string | IMarkdownString | undefined): IChatViewWelcomeContent {
		...
		if (this.input.currentModeKind === ChatModeKind.Ask) {
			...
			title = localize('agentTitle', "Build with LoCoPilot");
		}

		return {
			title,
			message: new MarkdownString(''),
			useBrandLogo: true, // Bixbite logo for "Build with LoCoPilot" (light/dark in CSS)
			additionalMessage,
	...
	private getPromptFileSuggestions(): IChatSuggestedPrompts[] {
		...
		// No predefined suggestions (Ask @vscode, Create Project, etc.) - keep chat welcome minimal
