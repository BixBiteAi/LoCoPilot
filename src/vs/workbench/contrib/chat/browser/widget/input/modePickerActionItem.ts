	...
	constructor(
		...
		const actionProvider: IActionWidgetDropdownActionProvider = {
			getActions: () => {
				...
				// Edit mode commented out from dropdown - only Ask and Agent are shown
				...
				const otherBuiltinModes = modes.builtin.filter(mode => mode.id !== ChatMode.Agent.id && mode.id !== ChatMode.Edit.id);
