				...
				diffIdentityProvider: {
					getId: (element) => {
						return element.dataId +
							...
							// Re-render when response parts change (e.g. tool invocations added or completed)
							(isResponseVM(element) && element.response?.value ? `_parts${element.response.value.length}` : '') +
