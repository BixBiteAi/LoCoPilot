...
export class ChatEditingService extends Disposable implements IChatEditingService {
	...
	private observerEditsInResponse(requestId: string, responseModel: IChatResponseModel, session: ChatEditingSession, observerDisposables: DisposableStore) {
		...
		// Run once immediately to process any parts already in the response (e.g. from edit tool
		// that ran and called acceptResponseProgress before we finished attaching the observer).
		// This ensures agent tool edits show up with diff colors, keep/undo, and "1 of n" navigation.
