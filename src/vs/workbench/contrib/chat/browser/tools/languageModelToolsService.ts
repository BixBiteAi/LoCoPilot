...
export class LanguageModelToolsService extends Disposable implements ILanguageModelToolsService {
	...
	constructor(
		...
		@ILoCoPilotAgentSettingsService private readonly _agentSettingsService: ILoCoPilotAgentSettingsService,
		@ILoCoPilotFileLog private readonly _locopilotFileLog: ILoCoPilotFileLog,
	) {
		super();
		...
		this.vscodeToolSet = this._register(this.createToolSet(
			...
			VSCodeToolReference.vscode,
			{
				...
				description: localize('copilot.toolSet.vscode.description', 'Use LoCoPilot features'),
			}
		));
	...
	async invokeTool(dto: IToolInvocation, countTokens: CountTokensCallback, token: CancellationToken): Promise<IToolResult> {
		...
		try {
			if (dto.context) {
				...
				this._logService.info(`[LoCoPilot Tool UI] getSession(${dto.context.sessionResource.toString()}): model=${model ? 'found' : 'null'}`);
				this._locopilotFileLog.log(`[LoCoPilot Tool UI] getSession(${dto.context.sessionResource.toString()}): model=${model ? 'found' : 'null'}`);
				if (!model) {
					throw new Error(`Tool called for unknown chat session`);
				}

				// Find the request by chatRequestId if provided (so we append to the correct response), otherwise use the last request
				const allRequestIds = model.getRequests().map(r => r.id);
				const request = (dto.chatRequestId ? model.getRequests().find(r => r.id === dto.chatRequestId) : undefined) ?? model.getRequests().at(-1);
				this._logService.info(`[LoCoPilot Tool UI] lookup request: chatRequestId=${dto.chatRequestId ?? 'undefined'}, requestIdsInSession=[${allRequestIds.join(', ')}], resolvedRequestId=${request?.id ?? 'null'}`);
				this._locopilotFileLog.log(`[LoCoPilot Tool UI] lookup request: chatRequestId=${dto.chatRequestId ?? 'undefined'}, requestIdsInSession=[${allRequestIds.join(', ')}], resolvedRequestId=${request?.id ?? 'null'}`);
				if (!request) {
					throw new Error(`Tool called but could not find chat request`);
				}
				...
				if (hadPendingInvocation && toolInvocation) {
					...
					this._logService.info(`[LoCoPilot Tool UI] appendProgress: requestId=${request.id}, toolId=${dto.toolId}, sessionResource=${dto.context.sessionResource.toString()}`);
					this._locopilotFileLog.log(`[LoCoPilot Tool UI] appendProgress: requestId=${request.id}, toolId=${dto.toolId}, sessionResource=${dto.context.sessionResource.toString()}`);
