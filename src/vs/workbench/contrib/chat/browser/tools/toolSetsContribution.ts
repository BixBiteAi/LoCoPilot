		...
		this._store.add(autorun(async r => {
			...
			for (const entry of entries) {
				...
				try {
					...
					// Treat empty or invalid JSON as empty tool set (no error)
					let rawObj: unknown = {};
					if (text) {
						try {
							rawObj = parse(text);
						} catch (parseErr) {
							this._logService.error(`Error reading tool set file ${entry.resource.toString()}:`, parseErr);
							continue;
						}
					}
					if (!isObject(rawObj)) {
						this._logService.error(`Tool set file ${entry.resource.toString()} must be a JSON object`);
						continue;
					}
