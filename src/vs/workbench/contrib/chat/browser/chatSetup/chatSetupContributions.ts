...
import { AICodeActionsHelper, AINewSymbolNamesProvider, ChatCodeActionsProvider, LoCoPilotBuiltInAgent, SetupAgent } from './chatSetupProviders.js';
		...
		const updateRegistration = () => {
			...
			// Agent + Tools
			{
				if (!context.state.hidden && !context.state.disabled) {
					...
					// Register LoCoPilot built-in agent (always available, will use existing agents when available)
					// This agent works without requiring extensions and delegates to existing agents when possible
					const defaultDisposables = defaultAgentDisposables.value;
					if (defaultDisposables && defaultDisposables instanceof DisposableStore) {
						const locopilotDisposables = new DisposableStore();
						// Register LoCoPilot agents for all locations and modes
						for (const mode of [ChatModeKind.Ask, ChatModeKind.Edit, ChatModeKind.Agent]) {
							locopilotDisposables.add(LoCoPilotBuiltInAgent.registerAgent(this.instantiationService, ChatAgentLocation.Chat, mode));
						}
						locopilotDisposables.add(LoCoPilotBuiltInAgent.registerAgent(this.instantiationService, ChatAgentLocation.Terminal, undefined));
						locopilotDisposables.add(LoCoPilotBuiltInAgent.registerAgent(this.instantiationService, ChatAgentLocation.Notebook, undefined));
						locopilotDisposables.add(LoCoPilotBuiltInAgent.registerAgent(this.instantiationService, ChatAgentLocation.EditorInline, undefined));
						defaultDisposables.add(locopilotDisposables);
					}
				} else {
